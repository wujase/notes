...
![HTTP Protocol](http-protocol.svg)
...
![Ajax Polling Protocol](ajax-polling-protocol.svg)
...
![HTTP Long-polling Protocol](http-long-polling-protocol.svg)
...
![WebSockets Protocol](websockets-protocol.svg)
...
3. SSEs are best when we need real-time traffic from the server to the client or if the server is generating data in a loop and will be sending multiple events to the client.

![Server-sent Events Protocol](server-sent-events-protocol.svg)
