...
Our service should be designed in a modular way with the expectation that new functionality will be added to it. There could be newer document types that need to be downloaded and processed in the future.
...
`15 billion pages / (4 weeks * 7 days * 86400 sec) ~= 6200 pages/sec`

### What about storage?

Page sizes vary a lot, but, as mentioned above since, we will be dealing with HTML text only, let’s assume an average page size of 100 KB. With each page, if we are storing 500 bytes of metadata, total storage we would need:
...
![A Bare Minimum Web Crawler](a-bare-minimum-web-crawler.svg)
...
![Detailed Component Design of a Web Crawler](detailed-component-design-of-a-web-crawler.svg)
